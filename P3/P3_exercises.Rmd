...
cd /Users/alexa/Library/CloudStorage/OneDrive-UniversitaetDuisburg-Essen/Master/UAB/TAB/bioinformatics-intro
mkdir P3
cd P3
mkdir data
cd data
...
# ggplot2 plot
ggplot(data = dataGroups, aes(x = Stage, fill = Group)) +
  geom_bar()
```
...
GeneID, lenght in basepairs and gene symbol
:::
...
On Genes: GeneID, lenght in bp, gene symbol, count of transcripts per patient

On patients: ID, stage, in "phenotype DF": total number of reads, count of mapped reads, sequencing platform, metadata on individuals (sex, ethnicity, alive/dead, country/state) details on tumor and current therapy (eg chemo, radiation, previous treatment responses). In total a huge amount if different data points is available on each patient. Many of which will not be used in our analysis (eg alcohol consumption or asbestos exposure)

In our data 87 individuals and 58037 are included. Staging reveals 26 early and 61 late stage individuals
:::
...
<i class="fa fa-question-circle"></i> **What type of object is** <ccode>res</code>?
...
```{r result1, eval = F}
nrow(res_tb[res_tb$pvalue < 0.001 &
              res_tb$log2FoldChange > 0,]) 
nrow(res_tb[res_tb$pvalue >= 0.001,]) 
nrow(res_tb[res_tb$pvalue < 0.001 &
...
When a p value of 0.001 is defined, 3357 genes are not significantly deferentially expressed. Of course this number would change with the use of a different p-value.

If any change in the expression is counted, 64 genes are underexpressed and 541 are overexpressed. When the treshhold is changed to a fold change of 1.5, 327 over and 22 underexpressed genes remain
:::
...
```{r ggplot1}
vulcano <- ggplot( data = res_tb, aes(y = -log10(padj), x = log2FoldChange, colour = filter)) +
  geom_point()
vulcano
```
...
```{r ggplot2}
vulcano +
  scale_colour_manual(values = c("FAlSE" = "grey", "TRUE" = "red"), name = " ", labels = c("high significance", "low")) +
  #geom_text(aes(label=ifelse(filter == "TRUE",as.character(hgnc_symbol),'')),hjust=0,vjust=0, size = 3) +
  theme(legend.position = "NONE") +
  theme_classic() +
  labs( x = "Expression change [log2]", y = "p-value [-log10]") +
  geom_label_repel(data=res_tb[abs(res_tb$log2FoldChange) > log2(10) & res_tb$padj < 0.001 ,], aes(label = as.factor(hgnc_symbol)), alpha = 0.7, size = 5, force = 1.3) 
```
...
# Execute GO analysis
egoBP <- enrichGO(gene = signif_genes,
...
egoMF <- enrichGO(gene = signif_genes,
                keyType = "SYMBOL", # change name to symbols
                OrgDb = org.Hs.eg.db,
                ont = "MF", # Biological process, molecular function (MF) and cell component (CC) available
                pAdjustMethod = "BH",
                qvalueCutoff = 0.05,
                readable = F)
...
egoCC <- enrichGO(gene = signif_genes,
                keyType = "SYMBOL", # change name to symbols
                OrgDb = org.Hs.eg.db,
                ont = "CC", # Biological process, molecular function (MF) and cell component (CC) available
                pAdjustMethod = "BH",
                qvalueCutoff = 0.05,
                readable = F)
```
...
dotplot(egoMF, showCategory=15) +
  theme(axis.text.y = element_text(size=8)) +
  labs(title = "molecular function")

dotplot(egoCC, showCategory=15) +
  theme(axis.text.y = element_text(size=8)) +
...
most up regulations relate to muscle function, even cellular compartment with sacromere etc.

Potential reasons are adaption to ECM (Cell stiffness for living in environment with high mechanical pressure (pleura is sheet of the lung)).

Might be due to EMT (epithelial to mesenchymal transformation), which is needed for tumor cells to be able to migrate and found metastasis.

CAF (Cancer activated fibroblasts) very important in progression of pleural cancer. They change tumor microenvironment and stiffen the ECM. A stiff ECM leads to the need of higher stiffness of cancer cells to withstand. This is for example achieved by up regulation of muscle related proteins
:::
...
Cytoskeleton as the KEGG pathway term with the highest Gene ratio supports the thesis that cancer cells of late stage cancers develop a higher stiffness due to the stiffer ECM.
:::
...
myPathway <- keggResult$ID[4]
```

```{r}
# MY OWN testing with gene data with FC 
# make table with genes + their FC value between early vs late
FCdata <- res_tb %>% filter(hgnc_symbol %in% ids$SYMBOL) %>% select(hgnc_symbol, log2FoldChange) %>% cbind(ids) %>% mutate(check = hgnc_symbol == SYMBOL)
# check if all are matched correctly
table(FCdata$check)
row.names(FCdata) <- FCdata$ENTREZID
input <- FCdata %>% select(log2FoldChange)
rFCdata = pathview(gene.data = input, pathway.id = myPathway, species = kegg_organism, same.layer = TRUE, limit=list(gene=6, cpd=1))
...
library(pathview)
r1 = pathview(gene.data = dedup_ids$ENTREZID, pathway.id = pathway, species = kegg_organism, same.layer = TRUE)
rMP = pathview(gene.data = dedup_ids$ENTREZID, pathway.id = myPathway, species = kegg_organism, same.layer = TRUE)
# QUESTION: legend appears only if same.layer = TRUE and changes coloring
