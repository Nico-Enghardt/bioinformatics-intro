...
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, shape = Species)) +
  geom_point()
# no error message for shape

ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, size = Species)) +
  geom_point()
# warning for size: using size for a discrete value is not adviced

ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, alpha = Species)) +
  geom_point()
# warning for alpha: using alpha for a discrete value is not adviced
...
# normalizes the number of cars to 100% and shows categories as share
```
:::
...
class(mtcars$gear)
mtcars$gear_f <- factor(mtcars$gear, levels = c(3, 4, 5))
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
# data is split into one diagram per category but displayed vertically instead of horizontal
```
:::
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  ggtitle("Diamond weight variation") +
  theme_bw()
#changes colours to ones suitable for colourblind people
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  ggtitle("Diamond weight variation") +
  theme_bw()
#changes colours to sequential shades (blue as default)
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  ggtitle("Diamond weight variation") +
  theme_bw()
#changes colours to ones suitable for black and white print
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  ggtitle("Diamond weight variation") +
  theme_bw()
#changes colours to shades of grey
```
:::
...
```{r answer5.2}
p + labs(subtitle = "bling bling", caption = "diamonds are a girls bestfriend", tag = "iced out")
# add anotation to the plot in a predifened location
```
:::
...
```{r answer5.3}
p + theme_minimal() #best dat-ink ratio
p + theme_classic() #lines missing to read numbers clearly
p + theme_bw() # okay but not all colour needed (eg gray boxes)
p + theme_dark() #bad
...
# saves as raster, resolution bad when zooming in 

BiocManager::install("svglite")
library(svglite)
ggsave(filename = "plot.svg", plot = p, width = 6, height = 4) 
# saves as vector, better image resolution when zooming in 
...
ggsave(filename = "us_unemployment.png", plot = e, width = 6, height = 4) 
...
ggplot(data = msleep) +
  geom_col(aes(x = vore, y = sleep_total, fill = conservation)) +
  labs(title = "Animal sleep times", y = "Sleep time in hours", x = NULL, caption = "Source: msleep") +
  theme_light() +
  scale_fill_viridis_d(name = "Conservation type", na.value = "grey80")
