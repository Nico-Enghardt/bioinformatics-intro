...
<center><img src="dataviz2019_11_4D10_30_45.png" width="40%"/></center>
...
<center><img src="bandtwo2.png" width="80%"/></center>
...
<center><img src="chunk2019_11_6D9_14_8.png" width="60%"/></center>
...
ggplot(d, aes(a, b, color = pc)) +
  geom_point(shape = 16, size = 2, show.legend = FALSE, alpha = 0.4) +
  theme_minimal() +
...
-   **Character**: consist of numbers, letters or words delimited with
    quotes (e.g. `"AGT"`, `"2"`)
    -   **Numeric**: consists of numbers such as integers (e.g. `1`,
        `-3`) or doubles (e.g. `0.5`, `-12.3`)
...
<center><img src="pies2019_11_4D11_52_30.jpg" style="padding: 0 15px; float: right;" width="25%"/></center>
...
<center><img src="data2ink2019_11_4D11_52_18.jpg" width="80%"/></center>
...
<center><img src="ggplot2layers2019_8_10D11_28_23.png" width="80%"/></center>
...
<center><img src="1_Hh53mOF4Xy4eORjLilKOwA.png" width="80%"/></center>
...
variables, referred as **statistical transformations**. One of them is
to count the occurrences of each value in a variable, which is precisely
...
ggplot(data = counts_by_cyl_data_frame, mapping = aes(x = cyl, y = number_of_cars)) + geom_col()
```
...
mtcars$gear_f <- as.factor(mtcars$gear)
...
The last plot with side by side dogded graphs is best, because the
distributions for all qualities can be seen clearly as well as their
relative class sizes. Each of the 3 information is nicely fitted into
one of the 4 channels (carat -\> x, count -\> y, quality -\> color /
...
ggplot(data = diamonds, mapping = aes(y = carat, x = color, fill=color)) +
  geom_boxplot()
```
:::
...
The standard pallete "viris_d" is friendly to colorblind people, because
each class has a distinct lightness.
:::
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(. ~ cut) +
  scale_fill_viridis_d() + 
  labs(title="Diamond weight variation", x="Weight in Karat", y = "Number", subtitle = "distinguished by quality", tag="Figure 1", caption="captioncaption captioncaption") + 
...
ggsave(filename = "saving_test.png", plot = p, width = 6, height = 4) # In inches by default
```
...
you observe when you zoom in them?
...
```{r answer 6.1}
ggsave(filename = "saving_test.svg", plot = p, width = 6, height = 4)
```
:::
...
p <- ggplot(data = economics, aes(x = date)) + 
  geom_line(aes(y=unemploy), color="darkred")
  labs(title="Unemployment in the US", x="Year", y="unemployed in thousend" ) 
p
ggsave(filename="us_unemployment.png", plot=p, width=6, height=4)
```
:::

<i class="fa fa-key"></i> The final image should look like this:

<center><img src="plot2019_11_5D10_50_1.png" width="50%"/></center>
...
</a> <a href="mpg2020_11_7D10_10_11.png" target="new">
...
head(msleep)
msleep$vore_f = as.factor(msleep$vore)

ggplot(msleep, mapping = aes(x = vore_f, y = sleep_total, fill=conservation)) + 
  scale_fill_viridis_d(na.value="grey80") + geom_col() + 
  labs(title="Animal sleep times", y="Sleep time in hours", caption="Source:msleep", x="")
```
...
ggplot(diamonds, mapping = aes(x = price, y = carat, colour=clarity_f)) + 
  geom_point() +
  scale_fill_viridis_d(name="Clarity") + 
  facet_grid(. ~ cut) +
  labs(title="Diamond Price per Carat", caption="Source:diamonds", x="Price in US dollars", y="Weight in carat")
```
...
ggplot(data = midwest, mapping = aes(x=area, y=poptotal, color=state_f, size=popdensity)) +
  geom_point() +
